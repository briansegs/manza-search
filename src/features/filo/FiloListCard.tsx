...
import { FiloListCardProps } from './types'

export function FiloListCard({ content, removeFn, pending, name, groupId }: FiloListCardProps) {
  async function handleRemove() {
    try {
      if (removeFn) {
        await removeFn({ contentId: content.id, listId: groupId })
        toast.success(`${name ? name : 'Content'} item removed!`)
      }
    } catch (error) {
